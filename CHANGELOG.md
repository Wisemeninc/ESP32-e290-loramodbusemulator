...
### Fixed
- OTA update check now triggers immediately when WiFi connects (was not checking on startup)

## [1.92] - 2026-01-14

### Changed
- Version bump to test OTA update with Core 1 fix
...
### Fixed
- **OTA Task Core Assignment**: Moved OTA task from Core 0 to Core 1
  - Root cause: WebServerTask on Core 0 performs CPU-intensive SSL handshakes
  - This starved the IDLE0 task on Core 0, triggering watchdog timeout
  - OTA task now runs on Core 1 where loopTask runs, avoiding conflict
  - Removed all watchdog monitoring from OTA task (not needed on Core 1)
  - Cleaner code without unnecessary esp_task_wdt_reset() calls
  - OTA and web server can now operate simultaneously without interference

## [1.89] - 2026-01-14

### Changed
- Version bump to test OTA update with improved download watchdog protection
...
### Changed
- Version bump to test OTA update with comprehensive watchdog protection
